...
const checkWholeTypes = (awaits, asyncConfig) => {
    ...
    const redAlert = (c, t) => {
        throw TypeError(`${c} is not type ${rightType[t]}`)
    }
    ...
    const proxy = new Proxy(singleArgs, {
        get() {
...
const combineAsyncError = (awaits, asyncConfig) => {
    ...
    const operations = {
        ...
        requestCallback: func => {
            const {
                requestCallback: { always, success, failed }
            } = config
    ...
    const nested = () => {
        doGlide.node = (function* () {
            while (doGlide.times < doGlide.len) yield noErrorAwait(tasks[doGlide.times++])
